...
import { Env, fetchOptions } from '~/app/lib/env'
import LoadingIndicator from '~/components/LoadingIndicator'
import { Tabs, TabsContent } from '~/components/ui/tabs'
import { rpcUrl } from '~/constants/chains'
import { ERC_20_TOKEN_LIST_BY_CHAIN } from '~/constants/erc20'

export default function Page() {
  const { data: env, isLoading, error } = useQuery(fetchOptions)
  const { address, isConnecting } = useAccount()

  if (error) return <div>Failed to load config</div>
  if (isLoading || !env || isConnecting) return <LoadingIndicator />
...
const ConnectedPage = ({ env }: { env: Env }) => {
  ...
  const defaultPublicClients = useMemo(() => {
    return Object.entries(rpcUrl(env.ALCHEMY_API_KEY)).reduce(
      (acc, [chainId, rpcData]) => {
